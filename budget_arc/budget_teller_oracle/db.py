...
def initialize_schema(conn: oracledb.Connection) -> list[str]:
    statements: list[tuple[str, str]] = [
        ...
        (
            "BUDGET_RAW_TELLER_ACCOUNTS",
            """
            CREATE TABLE BUDGET_RAW_TELLER_ACCOUNTS (
                RAW_ACCOUNT_EVENT_ID NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                USER_ID VARCHAR2(64) NOT NULL,
                CONNECTION_ID VARCHAR2(128) NOT NULL,
                PROVIDER_ACCOUNT_ID VARCHAR2(128) NOT NULL,
                PROVIDER_ENROLLMENT_ID VARCHAR2(128),
                ...
                LAST_FOUR VARCHAR2(16),
                STATUS VARCHAR2(32),
                RAW_HASH VARCHAR2(64) NOT NULL,
                FIRST_SEEN_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                RAW_JSON CLOB NOT NULL,
                CONSTRAINT BUDGET_RAW_ACCT_USER_FK FOREIGN KEY (USER_ID)
                    REFERENCES BUDGET_USERS (USER_ID),
                CONSTRAINT BUDGET_RAW_ACCT_UK UNIQUE (USER_ID, PROVIDER_ACCOUNT_ID, RAW_HASH)
            )
            """,
        ),
        (
            "BUDGET_RAW_TELLER_TRANSACTIONS",
            """
            CREATE TABLE BUDGET_RAW_TELLER_TRANSACTIONS (
                RAW_TRANSACTION_EVENT_ID NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                USER_ID VARCHAR2(64) NOT NULL,
                CONNECTION_ID VARCHAR2(128) NOT NULL,
                PROVIDER_ACCOUNT_ID VARCHAR2(128) NOT NULL,
                PROVIDER_TRANSACTION_ID VARCHAR2(128) NOT NULL,
                ...
                FIRST_SEEN_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                RAW_JSON CLOB NOT NULL,
                CONSTRAINT BUDGET_RAW_TXN_USER_FK FOREIGN KEY (USER_ID)
                    REFERENCES BUDGET_USERS (USER_ID),
                CONSTRAINT BUDGET_RAW_TXN_UK UNIQUE (USER_ID, PROVIDER_TRANSACTION_ID, RAW_HASH)
            )
            """,
        ),
        (
            ...
            CREATE TABLE BUDGET_CATEGORIES (
                ...
                CATEGORY_TYPE VARCHAR2(32) DEFAULT 'expense' NOT NULL,
                IS_SYSTEM NUMBER(1) DEFAULT 0 NOT NULL,
                STATUS VARCHAR2(32) DEFAULT 'ACTIVE' NOT NULL,
                CREATED_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                UPDATED_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                CONSTRAINT BUDGET_CATEGORIES_USER_FK FOREIGN KEY (USER_ID)
                    REFERENCES BUDGET_USERS (USER_ID),
                CONSTRAINT BUDGET_CATEGORIES_PARENT_FK FOREIGN KEY (PARENT_CATEGORY_ID)
        ...
        (
            ...
            CREATE TABLE BUDGET_MERCHANTS (
                ...
                CREATED_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                UPDATED_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                CONSTRAINT BUDGET_MERCHANTS_USER_FK FOREIGN KEY (USER_ID)
                    REFERENCES BUDGET_USERS (USER_ID)
            )
            """,
        ),
        (
            "BUDGET_TRANSACTION_RULES",
            """
            CREATE TABLE BUDGET_TRANSACTION_RULES (
                RULE_ID VARCHAR2(64) PRIMARY KEY,
                USER_ID VARCHAR2(64) NOT NULL,
                RULE_NAME VARCHAR2(256) NOT NULL,
                MATCH_TYPE VARCHAR2(32) NOT NULL,
                MATCH_VALUE VARCHAR2(1024) NOT NULL,
                MERCHANT_ID VARCHAR2(64),
                CATEGORY_ID VARCHAR2(64),
                PRIORITY NUMBER DEFAULT 100 NOT NULL,
                AUTO_APPLY NUMBER(1) DEFAULT 1 NOT NULL,
                STATUS VARCHAR2(32) DEFAULT 'ACTIVE' NOT NULL,
                CREATED_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                UPDATED_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                CONSTRAINT BUDGET_TXN_RULES_USER_FK FOREIGN KEY (USER_ID)
                    REFERENCES BUDGET_USERS (USER_ID),
                CONSTRAINT BUDGET_TXN_RULES_MERCH_FK FOREIGN KEY (MERCHANT_ID)
                    REFERENCES BUDGET_MERCHANTS (MERCHANT_ID),
                CONSTRAINT BUDGET_TXN_RULES_CAT_FK FOREIGN KEY (CATEGORY_ID)
                    REFERENCES BUDGET_CATEGORIES (CATEGORY_ID)
            )
            """,
        ),
        (
            "BUDGET_TRANSACTION_EDITS",
            """
            CREATE TABLE BUDGET_TRANSACTION_EDITS (
                EDIT_ID VARCHAR2(64) PRIMARY KEY,
                USER_ID VARCHAR2(64) NOT NULL,
                PROVIDER VARCHAR2(32) DEFAULT 'teller' NOT NULL,
                PROVIDER_TRANSACTION_ID VARCHAR2(128) NOT NULL,
                EDITED_TRANSACTION_DATE DATE,
                MERCHANT_ID VARCHAR2(64),
                EDITED_MERCHANT_NAME VARCHAR2(512),
                CATEGORY_ID VARCHAR2(64),
                SUBCATEGORY_ID VARCHAR2(64),
                REVIEWED_STATUS VARCHAR2(32),
                EXCLUDED_FROM_BUDGET NUMBER(1),
                EXCLUDED_FROM_CASH_FLOW NUMBER(1),
                NOTES VARCHAR2(1024),
                CREATED_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                UPDATED_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                CONSTRAINT BUDGET_TXN_EDITS_USER_FK FOREIGN KEY (USER_ID)
                    REFERENCES BUDGET_USERS (USER_ID),
                CONSTRAINT BUDGET_TXN_EDITS_MERCH_FK FOREIGN KEY (MERCHANT_ID)
                    REFERENCES BUDGET_MERCHANTS (MERCHANT_ID),
                CONSTRAINT BUDGET_TXN_EDITS_CAT_FK FOREIGN KEY (CATEGORY_ID)
                    REFERENCES BUDGET_CATEGORIES (CATEGORY_ID),
                CONSTRAINT BUDGET_TXN_EDITS_SUBCAT_FK FOREIGN KEY (SUBCATEGORY_ID)
                    REFERENCES BUDGET_CATEGORIES (CATEGORY_ID),
                CONSTRAINT BUDGET_TXN_EDITS_UK UNIQUE (USER_ID, PROVIDER, PROVIDER_TRANSACTION_ID)
            )
            """,
        ),
        (
            ...
            CREATE TABLE BUDGET_TRANSACTION_SPLITS (
                SPLIT_ID VARCHAR2(64) PRIMARY KEY,
                USER_ID VARCHAR2(64) NOT NULL,
                PROVIDER VARCHAR2(32) DEFAULT 'teller' NOT NULL,
                PROVIDER_TRANSACTION_ID VARCHAR2(128) NOT NULL,
                SPLIT_INDEX NUMBER NOT NULL,
                ...
                CREATED_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                UPDATED_AT TIMESTAMP WITH TIME ZONE DEFAULT SYSTIMESTAMP NOT NULL,
                CONSTRAINT BUDGET_TXN_SPLITS_USER_FK FOREIGN KEY (USER_ID)
                    REFERENCES BUDGET_USERS (USER_ID),
                CONSTRAINT BUDGET_TXN_SPLITS_MERCH_FK FOREIGN KEY (MERCHANT_ID)
                    REFERENCES BUDGET_MERCHANTS (MERCHANT_ID),
                CONSTRAINT BUDGET_TXN_SPLITS_CAT_FK FOREIGN KEY (CATEGORY_ID)
                    REFERENCES BUDGET_CATEGORIES (CATEGORY_ID),
                CONSTRAINT BUDGET_TXN_SPLITS_UK UNIQUE (USER_ID, PROVIDER, PROVIDER_TRANSACTION_ID, SPLIT_INDEX)
            )
            """,
        ),
        (
    ...
    with conn.cursor() as cur:
        ...
        if _table_exists(conn, "BUDGET_RAW_TELLER_ACCOUNTS") and _table_exists(conn, "BUDGET_ACCOUNTS"):
            cur.execute(
                """
                INSERT INTO BUDGET_RAW_TELLER_ACCOUNTS (
                    USER_ID, CONNECTION_ID, PROVIDER_ACCOUNT_ID, PROVIDER_ENROLLMENT_ID,
                    INSTITUTION_ID, INSTITUTION_NAME, ACCOUNT_NAME, ACCOUNT_TYPE,
                    ACCOUNT_SUBTYPE, CURRENCY_CODE, LAST_FOUR, STATUS, RAW_HASH,
                    FIRST_SEEN_AT, RAW_JSON
                )
                SELECT
                    a.USER_ID, a.CONNECTION_ID, a.PROVIDER_ACCOUNT_ID, a.PROVIDER_ENROLLMENT_ID,
                    a.INSTITUTION_ID, a.INSTITUTION_NAME, a.ACCOUNT_NAME, a.ACCOUNT_TYPE,
                    a.ACCOUNT_SUBTYPE, a.CURRENCY_CODE, a.LAST_FOUR, a.STATUS,
                    LOWER(RAWTOHEX(STANDARD_HASH(a.PROVIDER || ':' || a.PROVIDER_ACCOUNT_ID || ':v1-backfill', 'SHA256'))),
                    a.CREATED_AT, NVL(a.RAW_JSON, TO_CLOB('{}'))
        ...
        if _table_exists(conn, "BUDGET_RAW_TELLER_TRANSACTIONS") and _table_exists(conn, "BUDGET_TRANSACTIONS"):
            cur.execute(
                """
                INSERT INTO BUDGET_RAW_TELLER_TRANSACTIONS (
                    USER_ID, CONNECTION_ID, PROVIDER_ACCOUNT_ID, PROVIDER_TRANSACTION_ID,
                    INSTITUTION_ID, INSTITUTION_NAME, TRANSACTION_DATE, AMOUNT,
                    CURRENCY_CODE, DESCRIPTION, CATEGORY, STATUS, TRANSACTION_TYPE,
                    RAW_HASH, FIRST_SEEN_AT, RAW_JSON
                )
                SELECT
                    t.USER_ID, t.CONNECTION_ID, t.PROVIDER_ACCOUNT_ID, t.PROVIDER_TRANSACTION_ID,
                    t.INSTITUTION_ID, t.INSTITUTION_NAME, t.TRANSACTION_DATE, t.AMOUNT,
                    t.CURRENCY_CODE, t.DESCRIPTION, t.CATEGORY, t.STATUS, t.TRANSACTION_TYPE,
                    LOWER(RAWTOHEX(STANDARD_HASH(t.PROVIDER || ':' || t.PROVIDER_TRANSACTION_ID || ':v1-backfill', 'SHA256'))),
                    t.CREATED_AT, NVL(t.RAW_JSON, TO_CLOB('{}'))
        ...
        indexes = [
            ...
            "CREATE INDEX BUDGET_RAW_ACCT_USER_IDX ON BUDGET_RAW_TELLER_ACCOUNTS (USER_ID, PROVIDER_ACCOUNT_ID)",
...
class BudgetStore:
    ...
    def record_raw_teller_account(self, *, user_id: str, connection_id: str, account: dict[str, Any]) -> None:
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                ...
                INSERT INTO BUDGET_RAW_TELLER_ACCOUNTS (
                    USER_ID, CONNECTION_ID, PROVIDER_ACCOUNT_ID, PROVIDER_ENROLLMENT_ID,
                    INSTITUTION_ID, INSTITUTION_NAME, ACCOUNT_NAME, ACCOUNT_TYPE,
                    ACCOUNT_SUBTYPE, CURRENCY_CODE, LAST_FOUR, STATUS, RAW_HASH, RAW_JSON
                )
                SELECT
                    :user_id, :connection_id, :provider_account_id, :provider_enrollment_id,
                    :institution_id, :institution_name, :account_name, :account_type,
                ...
                WHERE NOT EXISTS (
                    SELECT 1
                    FROM BUDGET_RAW_TELLER_ACCOUNTS
                    WHERE USER_ID = :user_id
        ...
        *,
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                ...
                WHERE NOT EXISTS (
                    SELECT 1
                    FROM BUDGET_RAW_TELLER_TRANSACTIONS
                    WHERE USER_ID = :user_id
                ...
                raw_hash=_json_hash(transaction),
                raw_json=raw_json,
            )
        ...
        *,
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                ...
                SELECT
                    'teller', :provider_transaction_id, :user_id, :provider_account_id,
                    :connection_id, :institution_id, :institution_name, :transaction_date, :amount, :currency_code,
                    :description, :category, :counterparty_name, :counterparty_type,
                    :status, :transaction_type, :running_balance, :raw_json
                FROM dual
