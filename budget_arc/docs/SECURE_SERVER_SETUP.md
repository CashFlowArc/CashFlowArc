...
- Store SMTP credentials as server secrets, and use app-specific credentials rather than a primary email password.
- Store email verification and password reset tokens only as one-way hashes in Oracle.
...
| `BUDGET_SMTP_PASSWORD` | Email provider | Server env or secret store |
